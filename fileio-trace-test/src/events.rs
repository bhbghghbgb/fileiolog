...
/// Key: (opcode, version) - for kernel tracing, opcode identifies the event type
pub static EVENT_REGISTRY: Lazy<HashMap<(u8, u8), FileIoEventDef>> = Lazy::new(|| {
...
pub fn log_event(opcode: u8, event_id: u16, version: u8, _timestamp: u64, process_id: u32, thread_id: u32) {
    ...
    if let Some(known) = EVENT_REGISTRY.get(&key) {
        log::trace!(
            "FileIo Event: {} (opcode={}, Version={}, Class={}) PID={} TID={}",
            known.event_name, opcode, version, known.class_name, process_id, thread_id
        );
    } else {
